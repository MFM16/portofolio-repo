            ...
            <div class="flex flex-wrap -mx-3">
              <div class="items-center w-full max-w-full px-3 shrink-0">
                <div class="flex items-center mb-7">
                  <h6>{{ Auth::user()->profile->setting->language == 1 ? 'Available Icons List' : 'Daftar Ikon Tersedia'}}</h6>
                  <button type="button" class="btn-info ml-4" id="btn-modal">
                    <i class="fas fa-plus">&nbsp;</i></i>
                    {{ Auth::user()->profile->setting->language == 1 ? 'Add Icon' : 'Tambah Ikon'}}
                  </button>
                </div>
...
<div class="min-h-screen min-w-full h-full w-full backdrop-blur-md bg-transparent absolute z-990 top-0 flex items-center justify-center hidden" id="modal">
  <div class="modal-body">
    <div class="modal-header">
      <span class="modal-title">{{ Auth::user()->profile->setting->language == 1 ? 'Add New Icon' : 'Tambah Ikon Baru' }}</span>
    ...
    <form id="form-modal">
      <div>
        ...
        <div class="grup-input">
          <div class="flex">
            <span class="form-title">{{ Auth::user()->profile->setting->language == 1 ? 'Icon Name' : 'Nama Ikon' }}</span>
        ...
        <div class="grup-input">
          ...
          <input class="form-input" type="text" id="icon">
        </div>
      </div>
    </form>
    <div class="modal-footer">
      <button class="modal-button" type="button" id="btn-icon">
