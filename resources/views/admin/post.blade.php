            ...
            <div class="flex flex-wrap -mx-3">
              <div class="items-center w-full max-w-full px-3 shrink-0">
                <div class="flex items-center mb-7">
                  <h6>{{ Auth::user()->profile->setting->language == 1 ? "Post" : 'Artikel' }}</h6>
                  <button type="button" class="btn-info ml-4" id="btn-modal">
                    <i class="fas fa-plus">&nbsp;</i></i>{{ Auth::user()->profile->setting->language == 1 ? "Add Post" : 'Tambah Artikel' }}
                  </button>
                </div>
...
<div class="min-h-screen min-w-full h-full w-full backdrop-blur-md bg-transparent absolute z-990 top-0 flex items-center justify-center hidden" id="modal">
  <div class="modal-body">
    <div class="modal-header">
      <span class="modal-title">{{ Auth::user()->profile->setting->language == 1 ? "Add New Post" : 'Tambah Artikel Baru' }}</span>
    ...
    <form id="form-modal" enctype="multipart/form-data">
      <div>
        ...
        <div class="grup-input">
          <div class="flex">
            <span class="form-title">{{ Auth::user()->profile->setting->language == 1 ? "Post Title" : 'Judul Artikel' }}</span>
        ...
        <div class="grup-input">
          <div class="flex">
            <span class="form-title">{{ Auth::user()->profile->setting->language == 1 ? "Short Description Post" : 'Deskripsi Pendek Artikel' }}</span>
            <span class="text-xs text-red-500 ml-1">*</span>
          </div>
        ...
        <div class="grup-input">
          <div class="flex">
            <span class="form-title">{{ Auth::user()->profile->setting->language == 1 ? "Content" : 'Isi Artikel' }}</span>
            <span class="text-xs text-red-500 ml-1">*</span>
          </div>
    ...
    <div class="modal-footer">
      <button class="modal-button" type="button" id="btn-post">
    ...
    <script>
      ...
      class MyUploadAdapter {
        ...
        _initRequest() {
            ...
            // could be different.
            xhr.open( 'POST', '{{ url('admin/post/image') }}', true );
